...
// Aquí se indica en qué pines (cables) se conectaron los sensores de la puerta
...
// Estadísticas del día: cuántas personas entraron y salieron desde el último encendido
int totalEntradas = 0;
int totalSalidas  = 0;

// Variables que recuerdan el estado de cada sensor para detectar la dirección del movimiento
...
void cargarContador() {
  ...
  Serial.println("Contador recuperado de memoria: " + String(personas));
}
...
void verificarResetPorTiempo() {
  ...
  if (horas > 2.0) { // cambia 2.0 si necesitas más margen
    personas = 0;
    guardarContador();
    Serial.println("Reset por inactividad (" + String(horas, 1) + "h apagado)");
    enviarASheets("RESET"); // Se registra en Sheets para no confundirlo con datos reales
  }
}
...
// Formato: 2026-06-25T14:30:00 (sin espacios para que la URL no se rompa)
String getTimestamp() {
  struct tm t;
...
void tareaHTTP(void* param) {
  ...
  for (;;) { // Esto se repite por siempre
    ...
    if (xQueueReceive(colaHTTP, &ev, portMAX_DELAY) == pdTRUE) {
      ...
      client.setInsecure(); // Google rota sus certificados frecuentemente, por eso no los verificamos
...
void handleRoot() {
  ...
  html += F("<!DOCTYPE html><html><head>"
            ...
            "<meta http-equiv='refresh' content='3'>" // Se actualiza sola cada 3 segundos
            "<title>Biblioteca UAI</title>"
            "<style>"
            ...
            "max-width:380px;width:90%;box-shadow:0 8px 24px rgba(0,0,0,.1);text-align:center;}"
            "h1{margin:0 0 4px;font-size:20px;color:#555;font-weight:500;}"
            ".pct{font-size:80px;font-weight:700;color:#222;line-height:1.1;}"
            ".sub{font-size:15px;color:#aaa;margin:4px 0 16px;}"
            ".barra{background:#eee;border-radius:12px;height:22px;margin:16px 0;overflow:hidden;}"
            ".fill{height:100%;border-radius:12px;transition:width .4s;}"
            ".estado{font-size:20px;font-weight:600;margin:0;}"
            ".stats{margin-top:20px;font-size:14px;color:#888;border-top:1px solid #eee;padding-top:16px;}"
            ".hora{margin-top:10px;font-size:12px;color:#ccc;}"
            "</style></head><body><div class='card'>"
            "<h1>Biblioteca UAI</h1>");

  // Muestra el porcentaje como número principal (más honesto que el conteo exacto)
  html += "<div class='pct'>" + String((int)pct);
  html += "<span style='font-size:36px;color:#bbb'>%</span></div>";
  html += "<div class='sub'>Ocupación estimada</div>";
  ...
  // Mensaje de estado en el color correspondiente
  html += "<p class='estado' style='color:" + color + "'>" + estado + "</p>";

  // Estadísticas del día: flujo de personas desde el último encendido
  // Estos datos permiten tomar decisiones sobre el uso del espacio
  html += "<div class='stats'>";
  html += "&#8593; Entradas hoy: <b>" + String(totalEntradas) + "</b>";
  html += "&nbsp;&nbsp;|&nbsp;&nbsp;";
  html += "&#8595; Salidas hoy: <b>"  + String(totalSalidas)  + "</b>";
  html += "</div>";

  // Hora de la última actualización
  html += "<p class='hora'>Actualizado: " + getTimestamp() + "</p>";

  html += "</div></body></html>";

  // Envía la página completa al navegador del usuario
...
void setup() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    Serial.println("Abre esa IP en tu navegador para ver el dashboard");
    ...
    // Verifica si hay que reiniciar el contador por inactividad prolongada
...
void loop() {
  ...
  delay(40);  // El delay(40) entre mediciones evita que el eco de un sensor interfiera con el otro
  ...
  if (secuenciaEntrada && !detectaA && !detectaB) {
    if (personas < AFORO_MAX) { // Se suman siempre y cuando la biblioteca no esté colapsada
      personas++;
      gtotalEntradas++;      // Suma al contador de entradas del día
      guardarContador();    // Guarda en memoria interna por si se corta la luz
      Serial.println("¡Alguien entró! Ocupación: " + String((int)((personas*100.0)/AFORO_MAX)) + "%");
  ...
  if (secuenciaSalida && !detectaA && !detectaB) {
    if (personas > 0) { // Se resta solo si hay gente adentro (no puede haber personas negativas)
      personas--;
      totalSalidas++;       // Suma al contador de salidas del día
      guardarContador();    // Guarda en memoria interna
      Serial.println("¡Alguien salió! Ocupación: " + String((int)((personas*100.0)/AFORO_MAX)) + "%");
      enviarASheets("SALIDA");
    }
    // Se reinician los sensores para la siguiente persona
    activadoA = activadoB = secuenciaEntrada = secuenciaSalida = false;
  }
